...
if __name__=="__main__":
    ...
    # assuming the AA effect causes the Arctic temperature to increase by 1.0 deg (Avg)
    ...
    def lorenz84_AA(x, y, z, t, a = 0.25, b = 4.0, F = 8.0, G = 1.0, epsilon = 0.4, alpha = 0.04, beta = 0.01):
        ...
        T = 73
        omega = 2 * np.pi / T
        AA = alpha * np.cos(2 * omega * t) - alpha - np.sin(omega * t) * beta
        dx = - y**2 - z**2 - a * x + a * F * (1 + epsilon * np.cos(omega * t) + AA)
